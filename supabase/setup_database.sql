-- Complete Database Setup Script for Symplepass
-- This script safely handles existing tables and ensures a clean setup

-- ============================================
-- PART 1: CLEANUP (Safe for repeated execution)
-- ============================================

-- Disable triggers temporarily to avoid constraint issues during cleanup
DO $$
BEGIN
    -- Disable all triggers on existing tables
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_regulations') THEN
        ALTER TABLE event_regulations DISABLE TRIGGER ALL;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_faqs') THEN
        ALTER TABLE event_faqs DISABLE TRIGGER ALL;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_course_info') THEN
        ALTER TABLE event_course_info DISABLE TRIGGER ALL;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_kit_items') THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_organizers') THEN
...
DO $$
BEGIN
    -- Level 3: Tables with most dependencies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'registrations') THEN
    ...
    -- Level 2: Tables with some dependencies
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'event_categories') THEN
...
-- ============================================
-- PART 7: CREATE TRIGGER FUNCTIONS
-- ============================================
