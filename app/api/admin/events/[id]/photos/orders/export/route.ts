...
export async function GET(
  ...
  try {
    ...
    const rateLimitKey = `photo_orders_export:${userId}`
    ...
    // Get export data
    const exportData = await exportPhotoOrdersToCSV(eventId, filters)
    ...
    // Return CSV file
    const filename = `pedidos_fotos_${eventId.slice(0, 8)}_${new Date().toISOString().split('T')[0]}.csv`
    ...
    console.error('Error exporting photo orders:', error)
    return NextResponse.json(
